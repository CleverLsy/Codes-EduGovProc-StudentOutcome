...
cap log close
log using "$logfiles/1-01-022_Clean_CFPS_PersonVar.log",replace //创建新的日志文件，记录命令和输出结果

global CFPS "C:/Users/Yangzhihua/Nutstore/1/FriendsSpace/一平/信息化建设、家庭教育与城乡儿童发展/RawData/CFPS/RawData"
...
global id_all2 "pid  pid_a_f pid_a_m fid*  cid* countyid*  code countyname cityname  provname wave indsurvey*"    
...
* 定义需要的前缀列表
local prefixes "edu sch pedu parcon achcon parsty paroth kessler wz"

* 遍历每个前缀，动态获取变量名列表
foreach prefix of local prefixes {
    capture ds `prefix'*, local(varlist)
    if "`varlist'" != "" {
        global indivVarList2010 "$indivVarList2010 `varlist'"
        dis "Added variables for prefix `prefix': `varlist'"
    }
    else {
        dis "No variables found for prefix `prefix'"
    }
}
...
//方法二：
local edu: ds edu*
local sch: colnames sch*
local pedu: colnames pedu*
local parcon: colnames parcon*
local achcon: colnames achcon*
local parsty: colnames parsty*
local paroth: colnames paroth*
local kessler: colnames kessler*
local wz: colnames wz*
global indivVarList2010 = "edu sch pedu parcon achcon parsty paroth kessler wz studypr happy2 workdum marriage urban hukou han age indsurvey id_all gender birthy wordtest mathtest income"
...
*【家庭因素】
* 个人教育支出  
gen peduexp1 = wd5total_m //代答
gen peduexp2 = ks9total_m  //自答
gen peduexp = peduexp1 if age < 16
replace peduexp = peduexp2 if age >= 16
label var peduexp "个体教育支出加总（元/年）"

gen peduexp1_tutor = wd6total_m
gen peduexp2_tutor = ksa1total_m
gen peduexp_tutor = peduexp1_tutor if age < 16
replace peduexp_tutor = peduexp2_tutor if age >= 16
label var peduexp_tutor "个体教育支出-课外辅导/家教费（元/年）"
...
*【家庭因素】
* 个人教育支出  
gen peduexp1 = pd5total //代答
gen peduexp2 = ks9total_m  //自答
gen peduexp = peduexp1 if age < 16
replace peduexp = peduexp2 if age >= 16
label var peduexp "个体教育支出加总（元/年）"

gen peduexp1_tutor = pd503r
gen peduexp2_tutor = ksa1total_m
gen peduexp_tutor = peduexp1_tutor if age < 16
replace peduexp_tutor = peduexp2_tutor if age >= 16
label var peduexp_tutor "个体教育支出-课外辅导/家教费（元/年）"
...
* 个人教育支出  
gen peduexp1 = pd5total //代答
gen peduexp2 = wd5total  //自答
gen peduexp = peduexp1 if age < 16
replace peduexp = peduexp2 if age >= 16
label var peduexp "个体教育支出加总（元/年）"

gen peduexp1_tutor = pd503r
gen peduexp2_tutor = wd503r
gen peduexp_tutor = peduexp1_tutor if age < 16
replace peduexp_tutor = peduexp2_tutor if age >= 16
label var peduexp_tutor "个体教育支出-课外辅导/家教费（元/年）"
...
* 手动添加所有变量
global indivVarList2018 "pid fidbaseline fid10 fid12 fid14 fid16 fid18 fid20 cid countyid pid_a_f pid_a_m eduy eduyim peduexp1_tutor peduexp2_tutor peduexp_tutor peduexp peduexp2 peduexp1 schdum schstage schdorm schpub  wz301 wz302 parsty21 parsty22 parsty23 parsty24 parsty25 parsty26 studypr happy workdum marriage urban hukou han age indsurvey gender birthy wf501 wf502 income mathtest18 mathtest18_sc2 wordtest18 wordtest18_sc2 InternetDummy InternetMinute InternetStudyDummy InternetStudy_LastWeek    InternetStudyDaily InternetStudyFreq InternetWorkDummy InternetWorkDaily InternetWorkFreq InternetSocialDummy InternetSocialDaily InternetSocialFreq InternetEntertainmentDummy InternetEntertainment_LastWeek InternetEntertainmentDaily InternetEntertainmentFreq InternetShoppingDummy InternetShoppingDaily InternetShoppingFreq"
...
drop _merge
gen resppid =  waproxy//12年的没有【问师姐】
keep $id_all2 $indivVarList2018
order $id_all2 $indivVarList2018
...
tab hk20
tab hk20, nolabel
recode hk20 (1=1 "农业户口")(3  = 0 "非农户口")(-8 -2 -1 5 79 = .), gen(hukou)
label var hukou "户口类型,农业户口=1, 非农户口=0"
...
keep if indsurvey22>0  //2022年存在个人问卷
drop _merge
merge 1:1 pid using "$cfps2022childproxy", force update
keep if indsurvey22>0  //2022年存在个人问卷
drop _merge
tab indsurvey22
tab indsurvey22, nolabel //3 同时存在 4 只存在个人问卷 5 只存在少儿家长代答问卷 【22年没有indsurvey这个变量】
...
* 手动添加所有变量
global indivVarList2022 "pid fidbaseline fid10 fid12 fid14 fid16 fid18 fid20 cid countyid pid_f pid_m eduy eduyim peduexp1_tutor peduexp2_tutor peduexp_tutor peduexp peduexp2 peduexp1 schdum schstage schdorm schpub  wz301 wz302 parsty21 parsty22 parsty23 parsty24 parsty25 parsty26 paroth1 paroth2 paroth3 paroth4 paroth5 paroth6 paroth7 paroth8 studypr mathtest22 wf502 wf501 wordtest22 happy happy2 workdum marriage urban hukou han age indsurvey gender birthy wf501 wf502 emp_income InternetStudyMinute computerInternetMinute mobileInternetMinute InternetDummy InternetMinute InternetStudyDummy InternetStudy_LastWeek    InternetStudyDaily InternetEntertainmentDummy InternetEntertainment_LastWeek InternetEntertainmentDaily InternetShoppingDummy InternetShoppingDaily"
...
local parentsVarList = "pid age han hukou urban marriage workdum eduy eduyim paroth*"
...
use "$temp/CFPS_IndivLevel_2012.dta", clear
